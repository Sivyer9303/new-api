...
Standard commands live in `makefile` and `web/package.json`; the notes below are only the non-obvious gotchas. The startup update script already runs `go mod download` and `web/` `bun install`, so dependencies are refreshed before each session. Bun is installed at `/usr/local/bin/bun` (symlink into `~/.bun/bin`).

- **Backend build depends on the frontend build.** `main.go` uses `//go:embed web/dist` + `//go:embed web/dist/index.html`, so the Go server will not compile until `web/dist/index.html` exists. Run `make build-web` first (real UI), or for backend-only work create a placeholder: `mkdir -p web/dist && echo '<!doctype html><html></html>' > web/dist/index.html`. Note the running Go binary embeds `web/dist` at build time and does not need it at runtime.
- **Starting the rsbuild dev server (`bun run dev`) clears `web/dist`.** After running the frontend dev server, `go build .` / `go test ./...` will fail the root `main` package with `pattern web/dist: no matching files found` until you rebuild the frontend (`make build-web`) or recreate the placeholder above.
- **Do not run `go run main.go`.** The root `main` package is split across multiple `.go` files, so single-file `go run main.go` fails with `undefined: configureTrustedProxies` (the `makefile` `start-api` target has this bug). Use `go run .`, or `go build -o new-api . && ./new-api`.
- **Local dev services:** backend on `:3000` (defaults to zero-config SQLite `one-api.db` when `SQL_DSN` is unset); frontend dev server on `:5173` via `cd web && bun run dev -- --host 0.0.0.0 --port 5173`, which proxies `/api`, `/mj`, `/pg` to `:3000`. Open `http://localhost:5173` for UI development. Docker (`make dev-api`) is NOT required — SQLite works standalone.
- **HTTP dev auth:** start the backend with `SESSION_COOKIE_SECURE=false` for local HTTP; otherwise the refresh/logout OriginGuard blocks the `:5173`→`:3000` proxy.
- **First-run setup wizard:** a fresh database shows a setup wizard that creates the initial admin. The dev SQLite DB is seeded with admin `admin` / `admin123456`. Use `make reset-setup` to reset the wizard/admin state.
- **Side-drawer submit buttons in dev:** the TanStack Router devtools floating pill (bottom-right) overlaps the footer submit button (e.g. "Save changes") of create/edit side drawers. When automating UI clicks, the submit button lives at the bottom-right and may be covered — move the devtools widget or focus the button and press Enter.
- **Checks:** `go test ./...` (all pass), `cd web && bun run typecheck`, `cd web && bun run lint`. `bun run lint` currently reports some pre-existing errors in the repo; they are not environment problems.
